...
class SharedJsonBufferImpl extends Serializable {
    constructor(obj, optionsOrBuffer) {
        ...
        this.scratchPtr = 0; // Public for ArrayCursor access
        this.scratchCap = 0;
        this.scratchLen = 0;
        this.scratchStart = 0;
        this.objectHandler = {
    ...
    collectGarbage() {
        ...
        const relocate = (oldPtr, type) => {
            ...
            if (type === TYPE_STRING) {
                ...
                tempU8.set(this.u8.subarray(oldPtr + 4, oldPtr + 4 + len), newPtr + 4);
            ...
            if (actualType === TYPE_OBJECT) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    if (vType === TYPE_OBJECT || vType === TYPE_ARRAY ||
    ...
    readString(ptr) {
        ...
        if (len < 64) {
            ...
            for (; i < loopLimit; i += 4) {
                ...
                res += String.fromCharCode(chunk & 0xff, (chunk >> 8) & 0xff, (chunk >> 16) & 0xff, chunk >>> 24);
            }
    ...
    getProxyForPtr(ptr) {
        ...
        if (this.proxyCache.has(resolvedPtr)) {
            ...
            if (cached)
                return cached; // Return if still in memory
        }
        ...
        this.registry.register(proxy, target);
        // Store as WeakRef
    ...
    static [toDeserialized](data) {
        return new _a(null, data);
    }
}
_a = SharedJsonBufferImpl;
(() => {
    initConsoleHooks();
    register(7, _a);
})();
...
export const SharedJsonBuffer = SharedJsonBufferImpl;
